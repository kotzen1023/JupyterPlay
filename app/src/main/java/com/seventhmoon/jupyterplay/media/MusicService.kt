...
import com.seventhmoon.jupyterplay.media.extensions.flag
import com.seventhmoon.jupyterplay.media.library.*
import kotlinx.coroutines.*

open class MusicService : MediaBrowserServiceCompat() {
    private val mTAG = MusicService::class.java.name
    ...
    private fun removeNowPlayingNotification() {
        Log.d(mTAG, "removeNowPlayingNotification")
        stopForeground(true)
    }
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        override fun onMetadataChanged(metadata: MediaMetadataCompat?) {
            Log.d(mTAG, "onMetadataChanged")
            mediaController.playbackState?.let { updateNotification(it) }
        }

        override fun onPlaybackStateChanged(state: PlaybackStateCompat?) {
            Log.d(mTAG, "onPlaybackStateChanged")
            state?.let { updateNotification(it) }
        }

        private fun updateNotification(state: PlaybackStateCompat) {
            Log.d(mTAG, "updateNotification")
