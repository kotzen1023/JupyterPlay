package com.seventhmoon.jupyterplay.viewmodels
...
import com.seventhmoon.jupyterplay.fragments.NowPlayingFragment
import com.seventhmoon.jupyterplay.media.extensions.id
...
import com.seventhmoon.jupyterplay.media.extensions.isPrepared
import com.seventhmoon.jupyterplay.utils.Event
...
class MainActivityViewModel(
    ...
    private val mTAG = MainActivityViewModel::class.java.name

    val rootMediaId: LiveData<String> =
        Transformations.map(mediaSessionConnection.isConnected) { isConnected ->
            Log.d(mTAG, "isConnected => $isConnected")
            if (isConnected) {
                Log.e(mTAG, "mediaSessionConnection.rootMediaId = ${mediaSessionConnection.rootMediaId}")
    ...
    fun mediaItemClicked(clickedItem: MediaItemData) {
        Log.d(mTAG, "mediaItemClicked: clickedItem.browsable = ${clickedItem.browsable}")
    ...
    private fun browseToItem(mediaItem: MediaItemData) {
        Log.d(mTAG, "browseToItem: mediaItem.mediaId = ${mediaItem.mediaId}")
    ...
    fun playMedia(mediaItem: MediaItemData, pauseAllowed: Boolean = true) {
        ...
        Log.e(mTAG, "playMedia: nowPlaying = $nowPlaying, isPrepared = $isPrepared")
    ...
    fun playMediaId(mediaId: String) {
        ...
        Log.e(mTAG, "playMediaId: nowPlaying = $nowPlaying, isPrepared = $isPrepared")
